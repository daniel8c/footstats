...
def situation_to_database():
    ...
    for row_1, row_2 in zip(data_home, data_away):
        insert_row(connection, cursor, row_1, 'situation_in_match')
        insert_row(connection, cursor, row_2, 'situation_in_match')
    for row_1, row_2 in zip(list(rosters_home.values()), list(rosters_home.values())):
        row_1['match_id'] = str(id)
        row_2['match_id'] = str(id)
        insert_row(connection, cursor, row_1, 'rosters')
        insert_row(connection, cursor, row_2, 'rosters')


def match_to_database():
    insert_row(connection, cursor, match, 'match_result')


def calc_match_id():
    with open(r"C:\footstats\log\id_match.txt", 'r') as f:
        existing_match_id = [x.rstrip() for x in f.readlines()]
